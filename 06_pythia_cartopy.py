...
ax = plt.subplot(1, 1, 1, projection=ccrs.InterruptedGoodeHomolosine())# projection=ccrs.PlateCarree(central_longitude=-75))
...
# %%
fig = plt.figure(figsize=(11, 8.5))
projLae = ccrs.LambertAzimuthalEqualArea(central_longitude=0.0)
...
#Plot Europe on the second subplots on a LambertConformal projection. Include the coastline, countries and lakes
...
ax = plt.subplot(1, 1, 1, projection=projMoll)
ax.coastlines()
dataplot = ax.pcolormesh(lon, lat, data, transform=ccrs.PlateCarree(), cmap='Reds')
...
ax = plt.subplot(1, 1, 1, projection=projMoll)
ax.coastlines()
dataplot = ax.scatter(lon, lat, c=data, transform=ccrs.PlateCarree(), cmap='Reds')
plt.colorbar(dataplot, orientation='horizontal');
...
# %%
uw = np.sqrt(ds['u-component_of_wind_isobaric']**2 + ds['v-component_of_wind_isobaric']**2)
uw
